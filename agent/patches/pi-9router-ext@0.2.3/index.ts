...
// =============================================================================
// Static models.yml overrides
//
// The router's /v1/models reports no limits for combo routes, so discovered
// models fall back to FALLBACK_CONTEXT_WINDOW (128K) unless models.dev has an
// entry. Users who define the same model id in the omp models.yml (e.g. the
// 9router provider's `models` list with an explicit contextWindow/maxTokens)
// expect that definition to win. Read those static definitions and prefer them
// over the router/metadata/fallback chain below.
// =============================================================================
...
const STATIC_MODELS_YML_PATH = join(
	process.env.PI_CODING_AGENT_DIR
		|| join(process.env.PI_CONFIG_DIR || join(homedir(), ".omp"), "agent"),
	"models.yml",
);

let staticModelsCache: Map<string, Map<string, StaticModelDefinition>> | undefined;
...
function parseStaticModelsYml(content: string): Map<string, Map<string, StaticModelDefinition>> {
	const providers = new Map<string, Map<string, StaticModelDefinition>>();
	let currentProvider: string | undefined;
	let modelsByProvider: Map<string, StaticModelDefinition> | undefined;
	let currentModel: StaticModelDefinition | undefined;
	...
	for (const raw of content.split("\n")) {
		...
		// Top-level provider boundary (indent 2, e.g. "  9router:").
		if (indent === 2 && text.endsWith(":")) {
			if (currentProvider !== undefined && modelsByProvider !== undefined) {
				providers.set(currentProvider, modelsByProvider);
			}
		...
		// Provider-level "models:" list.
		if (currentProvider !== undefined && text === "models:") {
			inModelsList = true;
			currentModel = undefined;
			thinkingBlock = false;
			continue;
		...
		if (text.startsWith("- id:")) {
			const id = text.slice("- id:".length).trim();
			currentModel = { id };
			modelsByProvider ??= new Map();
			modelsByProvider.set(id, currentModel);
			thinkingBlock = false;
			continue;
		}
		if (!currentModel) continue;

		// Left the models list (dedent below model entries).
		if (indent <= 6) {
			inModelsList = false;
			currentModel = undefined;
			thinkingBlock = false;
		...
		if (thinkingBlock && text.startsWith("efforts:")) {
			currentModel.thinking = {
				mode: "effort",
				efforts: parseStringList(text.slice("efforts:".length)),
			};
			continue;
		}
		if (thinkingBlock && indent <= 8) {
			thinkingBlock = false;
		}
		...
		switch (key) {
			...
			case "contextWindow": {
				...
				if (Number.isFinite(n)) currentModel.contextWindow = n;
				break;
			}
			case "maxTokens": {
				const n = Number(value);
				if (Number.isFinite(n)) currentModel.maxTokens = n;
				break;
			}
			case "input":
				currentModel.input = parseStringList(value) as ("text" | "image")[];
				break;
		}
	}
...
function loadStaticModelDefinitions(): Map<string, Map<string, StaticModelDefinition>> {
	if (staticModelsCache) return staticModelsCache;
	try {
		if (existsSync(STATIC_MODELS_YML_PATH)) {
			staticModelsCache = parseStaticModelsYml(readFileSync(STATIC_MODELS_YML_PATH, "utf8"));
		}
	} catch (err) {
		console.warn(`[pi-9router-ext] Failed to read static models.yml: ${errorMessage(err)}`);
	}
	return staticModelsCache ?? new Map();
}

function mapNineRouterModel(model: NineRouterModel, enableReasoning: boolean, metadata?: ModelMetadata) {
	const isCombo = model.owned_by === "combo";
	// Static definitions for the 9router provider win over the router's bare
	// combo entries, which carry no context/output limits.
	const staticDef = loadStaticModelDefinitions().get("9router")?.get(model.id);
	const contextWindow = staticDef?.contextWindow ?? modelContextWindow(model, metadata);
	const maxTokens = staticDef?.maxTokens ?? modelMaxTokens(model, metadata, contextWindow);

	return {
		id: model.id,
		name: staticDef?.name ?? (isCombo ? `🔀 ${model.id}` : model.id),
		...
		input: staticDef?.input ?? modelInputTypes(metadata),
...
function registerNineRouterProvider(
	...
	// Keep statically configured models available even when 9router omits them
	// from /v1/models. This is required for routes such as oc/x-preview-f-free
	// that are callable but not exposed by discovery.
	const staticModels = loadStaticModelDefinitions().get("9router");
	const registeredModels = [...models];
	const discoveredIds = new Set(models.map((model) => model.id));
	for (const definition of staticModels?.values() ?? []) {
		if (discoveredIds.has(definition.id)) continue;
	...
	pi.registerProvider("9router", {
		...
		models: registeredModels.map((model) => mapNineRouterModel(
			model,
