 ...
 *   OMNIROUTE_URL       — OmniRoute API URL. Resolution order: env var, then the
 *                         `omni` provider baseUrl from the omp models.yml, then the
 *                         Pi models.json omni baseUrl, then http://127.0.0.1:20128.
 *   OMNIROUTE_DASHBOARD — Dashboard URL shown to user (default: same resolution,
...
const OMNI_URL = process.env.OMNIROUTE_URL || omniProviderBaseUrl() || "http://127.0.0.1:20128";
const DASHBOARD_URL = process.env.OMNIROUTE_DASHBOARD || omniProviderBaseUrl() || "http://localhost:20128";
...
/**
 * omp agent dir: PI_CODING_AGENT_DIR, else PI_CONFIG_DIR, else ~/.omp/agent
 * if it exists. OmniRoute endpoints under omp (models.yml, no models.json).
 */
function ompAgentDir(): string | undefined {
...
/**
 * Resolve the OmniRoute base URL from config so the extension follows the
 * port the user actually runs OmniRoute on (this repo pins 20129 because
 * 9router owns 20128). Order: omp models.yml omni provider baseUrl, then Pi
 * models.json omni provider baseUrl, then undefined (caller falls back).
 */
function omniProviderBaseUrl(): string | undefined {
	try {
		const agentDir = ompAgentDir();
		if (agentDir) {
			const modelsYml = readFileSync(join(agentDir, "models.yml"), "utf8");
			const block = modelsYml.match(/^  omni:\n([\s\S]*?)(?=^  \S|\n\S)/m)?.[1];
			const base = block?.match(/^    baseUrl:\s*(\S+)/m)?.[1];
			if (base) return base.replace(/\/+$/, "");
		}
	} catch {}
	try {
		const p = process.env.PI_HOME
			? `${process.env.PI_HOME}/models.json`
			: join(homedir(), ".pi", "agent", "models.json");
		if (existsSync(p)) {
			const data = JSON.parse(readFileSync(p, "utf8"));
			const base = data?.providers?.omni?.baseUrl;
...
/**
 * omp models.yml path, or undefined when running against Pi's models.json.
 */
function ompModelsYmlPath(): string | undefined {
	const dir = ompAgentDir();
	return dir ? join(dir, "models.yml") : undefined;
}

/** One omp models.yml entry for an OmniRoute model/ combo. */
function omniModelYaml(m: OmniSyncModel): string {
	const lines = [
		`      - id: "${m.id}"`,
		`        name: "${m.name}"`,
	...
	lines.push("        thinking:", "          mode: effort", "          efforts: [low, medium, high, max]");
	return lines.join("\n");
}

/** Number of omni models currently declared in an omp models.yml. */
function omniModelsCountInYml(content: string): number {
	const m = content.match(/^  omni:[\s\S]*?^    models:\n([\s\S]*?)(?=^  \S)/m);
	if (!m) return 0;
	return (m[1].match(/^      - id:/gm) || []).length;
}

/**
 * Replace the `omni` provider's models list in an omp models.yml, preserving
 * provider-level keys (baseUrl, apiKey, authHeader) and every other provider.
 * Returns the updated YAML. Mirrors what /omni sync does in Pi's models.json.
 */
function applyOmniModelsToYml(content: string, models: OmniSyncModel[]): string {
	const start = content.search(/^  omni:\n/m);
	if (start === -1) {
		const anchors = content.match(/^(providers:\n)/m);
		const insertAt = anchors ? anchors.index! + anchors[1].length : 0;
		const block = `  omni:\n    baseUrl: ${OMNI_URL}\n    models:\n${models.map(omniModelYaml).join("\n")}\n`;
		return content.slice(0, insertAt) + block + content.slice(insertAt);
	}
	const after = content.slice(start + "  omni:\n".length);
	const blockEndMatch = after.search(/^  \S/m);
	const blockEnd = blockEndMatch === -1 ? content.length : start + "  omni:\n".length + blockEndMatch;
	const omniBlock = content.slice(start, blockEnd);
	const match = omniBlock.match(/^([\s\S]*?^    models:\n)/m);
	const head = match ? match[1] : `${omniBlock}    models:\n`;
	return content.slice(0, start) + head + models.map(omniModelYaml).join("\n") + "\n" + content.slice(blockEnd);
}
// ────────────────────────── health ──────────────────────────
...
async function getAllModelsFromOmniRoute(): Promise<OmniSyncModel[]> {
	const results: OmniSyncModel[] = [];
	...
	try {
		...
		for (const m of models) {
			...
			results.push({
				id,
				name: humanName(id),
				...(typeof rec.context_length === "number" ? { contextWindow: rec.context_length } : {}),
				...(typeof rec.max_output_tokens === "number" ? { maxTokens: rec.max_output_tokens } : {}),
			});
		}
...
export default function (pi: ExtensionAPI) {
	...
	pi.registerCommand("omni", {
		...
		async handler(args, ctx) {
			...
			if (sub === "sync") {
				ctx.ui.notify("Syncing models from OmniRoute to the model picker...", "info");

				try {
					const allModels = await getAllModelsFromOmniRoute();
					const fs = require("fs");
					const modelsYml = ompModelsYmlPath();

					// omp: rewrite providers.omni.models in models.yml (YAML) — omp
					// never reads Pi's models.json, so the legacy JSON path below
					// would silently update a file the picker ignores.
					if (modelsYml && existsSync(modelsYml)) {
						const oldCount = omniModelsCountInYml(fs.readFileSync(modelsYml, "utf8"));
						fs.writeFileSync(modelsYml, applyOmniModelsToYml(fs.readFileSync(modelsYml, "utf8"), allModels));
						ctx.modelRegistry.refresh();
						ctx.ui.notify(
							`✅ Synced ${allModels.length} omni models to ${modelsYml} (was ${oldCount})`,
							"info"
						);
						return;
					}
...
export {
	omniProviderBaseUrl,
	ompModelsYmlPath,
	applyOmniModelsToYml,
	omniModelsCountInYml,
	omniModelYaml,
};
