#!/usr/bin/env bash
# Launch omp with the Notion MCP server enabled for this run only.
#
# OMP has no native `--mcp` / `--notion-mcp` flag, and `--config` overlays only
# carry config.yml-style settings, not mcpServers. So this wrapper stages a
# temporary agent dir (symlinks to the live dir plus a merged mcp.json) and
# points OMP at it via PI_CODING_AGENT_DIR. The live mcp.json stays Notion-free.
#
# Usage: scripts/omp-notion.sh [omp args...]
#   e.g. scripts/omp-notion.sh
#   e.g. scripts/omp-notion.sh -p "summarize my Notion inbox"
#
# Suggested alias: alias ompn='<repo>/scripts/omp-notion.sh'
set -euo pipefail

REPO_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
BASE="${OMP_AGENT_DIR:-${PI_CODING_AGENT_DIR:-$HOME/.omp/agent}}"
FRAGMENT="$REPO_DIR/agent/mcp.notion.json"
OMP_BIN="${OMP_BIN:-omp}"
...
STAGE="$(mktemp -d "${TMPDIR:-/tmp}/omp-notion.XXXXXX")"
cleanup() { rm -rf "$STAGE"; }
trap cleanup EXIT
...
disabled = [s for s in (base.get("disabledServers") or []) if s != "notion"]
if disabled:
